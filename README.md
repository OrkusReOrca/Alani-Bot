...
- `npm start` reads `programs.json`, compares it against `data/state.json`
  ...
  - **Always** posts the full status list to the channel (via webhook and/or
    bot), tagging any changed/new entries with 🆕.
...
|---|---|
| `DISCORD_WEBHOOK_URL` | Discord → Channel Settings → Integrations → Webhooks. **Regenerate it** if the old URL was ever pasted into a chat. |
...
| `DISCORD_CHANNEL_ID` | Optional — only if you want the bot (not the webhook) to post to a channel. Right-click the channel → Copy Channel ID |

`.env` is already in `.gitignore` — it will never be committed.

You only need **one** delivery method to work (webhook OR bot DM), but you
can set up both.
...
2. In the repo, go to **Settings → Secrets and variables → Actions** and add
   ...
   - `DISCORD_BOT_TOKEN`
   - `DISCORD_USER_ID`
   - `DISCORD_CHANNEL_ID` (only if used)
